...
import { useContractorSafetyTalks } from "@/hooks/use-contractor-safety-talks"
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Ocurrió un error al cargar las charlas de seguridad
          </CardDescription>
        </CardHeader>
        <CardFooter>
        ...
        {isLoading ? (
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {data.availableTalks.map((talk) => (
              <Card key={talk.id} className="overflow-hidden flex flex-col">
                <CardHeader className="pb-3">
                  <div className="flex justify-between items-start">
                    <CardTitle className="text-lg">{talk.title}</CardTitle>
                    <Badge 
                      variant="outline" 
                      className={talk.isPresential 
                  ...
                  <CardDescription>
                    {talk.description || "Sin descripción"}
                  </CardDescription>
                </CardHeader>
                <CardContent className="flex-grow space-y-2 text-sm">
                  ...
                  <div className="flex items-center text-muted-foreground">
                    ...
                    <span>Vence: {format(new Date(talk.expiresAt), "dd 'de' MMMM, yyyy", { locale: es })}</span>
                  </div>
                </CardContent>
                <CardFooter>
                  {talk.isPresential ? (
              ...
              <Card key={talk.id} className="overflow-hidden flex flex-col">
                <CardHeader className="pb-3">
                  <div className="flex justify-between items-start">
                    <CardTitle className="text-lg">{talk.title}</CardTitle>
                    <Badge 
                      variant={talk.userSafetyTalk?.passed ? "default" : "destructive"}
                    >
                  ...
                  {talk.userSafetyTalk && (
                    <>
                      <div className="flex items-center text-muted-foreground">
                        ...
                        <span>Completada: {format(new Date(talk.userSafetyTalk.completedAt), "dd/MM/yyyy", { locale: es })}</span>
                      </div>
                      <div className="flex items-center text-muted-foreground">
                        <Calendar className="mr-2 h-4 w-4" />
                        <span>Válida hasta: {format(new Date(talk.userSafetyTalk.expiresAt), "dd/MM/yyyy", { locale: es })}</span>
                      </div>
                    </>
                  )}
                </CardContent>
