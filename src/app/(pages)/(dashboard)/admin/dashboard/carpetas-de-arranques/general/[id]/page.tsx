...
import { ChevronLeft, FileText, CheckCircle2, AlertCircle, Clock, X } from "lucide-react"
...
import { GeneralStartupFolderReviewForm } from "@/components/sections/startup-folders/GeneralStartupFolderReviewForm"
import { GeneralStartupFolderSubmitForm } from "@/components/sections/startup-folders/GeneralStartupFolderSubmitForm"
...
export default function GeneralStartupFolderPage() {
	...
	const isCompanySupervisor =
		session?.user?.role === "COMPANY_SUPERVISOR" && session?.user?.companyId === folder?.companyId
				...
				<div>
					<div className="flex items-center space-x-2">
						<h1 className="text-2xl font-bold tracking-tight">
							Carpeta de Arranque General: {folder.company.name}
						</h1>
						<StartupFolderStatusBadge status={folder.status} />
					</div>
					<p className="text-muted-foreground">
						{folder.company.rut} • {folder.company.name}
					</p>
				</div>
			...
			>
				...
				{canReview && (
					<TabsContent value="revisar" className="mt-6">
						<GeneralStartupFolderReviewForm folder={folder} userId={session?.user?.id || ""} />
					</TabsContent>
				)}
			</Tabs>
		</div>
