...
export async function GET() {
	...
	try {
		...
		// Module Activity Area Chart - Last 30 days
		const thirtyDaysAgo = subDays(new Date(), 30)

		const moduleActivityLogs = await prisma.activityLog.findMany({
			where: {
				timestamp: {
					gte: thirtyDaysAgo,
				},
		...
		// Group activity by date and module
		...
		for (let i = 0; i < 30; i++) {
			...
			activityByDate[dateKey] = {
				...
				company: 0,
				users: 0,
				laborControlFolders: 0,
			}
		}

		// Count activities by date and module
		moduleActivityLogs.forEach((log) => {
			const dateKey = format(log.timestamp, "dd/MM")
			if (activityByDate[dateKey]) {
				switch (log.module) {
					case MODULES.WORK_ORDERS:
						activityByDate[dateKey].workOrders++
						break
					case MODULES.WORK_PERMITS:
						activityByDate[dateKey].workPermits++
						break
					case MODULES.MAINTENANCE_PLANS:
						activityByDate[dateKey].maintenance++
						break
					case MODULES.EQUIPMENT:
						activityByDate[dateKey].equipment++
						break
					case MODULES.STARTUP_FOLDERS:
						activityByDate[dateKey].startupFolders++
						break
					case MODULES.SAFETY_TALK:
						activityByDate[dateKey].safetyTalk++
						break
					case MODULES.LOCKOUT_PERMITS:
						activityByDate[dateKey].lockoutPermits++
						break
					case MODULES.DOCUMENTATION:
						activityByDate[dateKey].documentation++
						break
					case MODULES.WORK_REQUESTS:
						activityByDate[dateKey].workRequests++
						break
					case MODULES.COMPANY:
						activityByDate[dateKey].company++
						break
					case MODULES.USERS:
						activityByDate[dateKey].users++
						break
					case MODULES.LABOR_CONTROL_FOLDERS:
						activityByDate[dateKey].laborControlFolders++
						break
				}
			}
		})

		const moduleActivityAreaChart = Object.entries(activityByDate).map(([date, modules]) => ({
			date,
			...modules,
		}))
