...
import { MODULES, ACTIVITY_TYPE, ACCESS_ROLE, LABOR_CONTROL_STATUS } from "@/generated/prisma/enums"
...
export async function GET(): Promise<NextResponse> {
	try {
		...
		console.log(
			`[CRON_LABOR_CONTROL] Found ${companies.length} active companies with startup folders`
		)
		...
		for (const company of companies) {
			try {
				...
				if (!laborControlFolder) {
					...
					console.log(
						`[CRON_LABOR_CONTROL] Created labor control folder for ${company.name} - ${currentMonthYear}`
					)
				} else {
					console.log(
						`[CRON_LABOR_CONTROL] Labor control folder already exists for ${company.name} - ${currentMonthYear}`
					)
				}
				...
				console.error(
					`[CRON_LABOR_CONTROL] Error processing company ${company.name}:`,
					companyError
				)
				results.push({
					success: false,
