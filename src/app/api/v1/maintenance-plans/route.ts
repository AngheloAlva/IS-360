...
import { maintenancePlanTaskSelect, flattenMaintenancePlanTask } from "@/lib/api-v1/mappers/maintenance-plan"
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const [records, total] = await Promise.all([
      db.maintenancePlanTask.findMany({
        where,
        take: limit + 1,
        ...(cursor ? { cursor: { id: cursor }, skip: 1 } : {}),
        orderBy: { id: "asc" },
        select: maintenancePlanTaskSelect,
      }),
      db.maintenancePlanTask.count({ where }),
    ])
    ...
    return buildPaginatedResponse({
      data: data.map(flattenMaintenancePlanTask),
    ...
    console.error("[API_V1_MAINTENANCE_PLANS]", error)
