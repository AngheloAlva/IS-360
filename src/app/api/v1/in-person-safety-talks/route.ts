...
import { inPersonSafetyTalkSelect, flattenInPersonSafetyTalk } from "@/lib/api-v1/mappers/in-person-safety-talk"
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const [records, total] = await Promise.all([
      db.inPersonSafetyTalkRecord.findMany({
        where,
        take: limit + 1,
        ...(cursor ? { cursor: { id: cursor }, skip: 1 } : {}),
        orderBy: { id: "asc" },
        select: inPersonSafetyTalkSelect,
      }),
      db.inPersonSafetyTalkRecord.count({ where }),
    ])
    ...
    return buildPaginatedResponse({
      data: data.map(flattenInPersonSafetyTalk),
    ...
    console.error("[API_V1_IN_PERSON_SAFETY_TALKS]", error)
