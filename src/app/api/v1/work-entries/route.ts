...
import { workEntrySelect, flattenWorkEntry } from "@/lib/api-v1/mappers/work-entry"
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const where = updatedSince ? { createdAt: { gte: updatedSince } } : {}
    ...
    return buildPaginatedResponse({
      data: data.map(flattenWorkEntry),
    ...
    console.error("[API_V1_WORK_ENTRIES]", error)
