...
import { safetyTalkSelect, flattenSafetyTalk } from "@/lib/api-v1/mappers/safety-talk"
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const [records, total] = await Promise.all([
      db.userSafetyTalk.findMany({
        where,
        take: limit + 1,
        ...(cursor ? { cursor: { id: cursor }, skip: 1 } : {}),
        orderBy: { id: "asc" },
        select: safetyTalkSelect,
      }),
      db.userSafetyTalk.count({ where }),
    ])
    ...
    return buildPaginatedResponse({
      data: data.map(flattenSafetyTalk),
    ...
    console.error("[API_V1_SAFETY_TALKS]", error)
