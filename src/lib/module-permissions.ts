...
export const MODULE_ROUTE_MAP: Record<MODULES | string, string[]> = {
	...
	STARTUP_FOLDERS: ["/admin/dashboard/carpetas-de-arranque"],
	...
	PROGRAMATION: ["/admin/dashboard/programacion"],
	INDICATORS: ["/admin/dashboard/indicadores"],
	EQUIPMENT_HISTORY: ["/admin/dashboard/historial-equipos"],
	CONTACT: [],
	NONE: [],
}
...
export function canAccessAdminRoute(userModules: (MODULES | string)[], route: string): boolean {
	...
	// Soporte es visible para todos los usuarios admin
	if (route.includes("/admin/dashboard/soporte")) {
		return true
	}
...
export function getBestRedirectRoute(userModules: (MODULES | string)[]): string {
	...
	const priorityRoutes = [
		...
		{ module: "STARTUP_FOLDERS", route: "/admin/dashboard/carpetas-de-arranque" },
