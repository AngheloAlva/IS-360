...
import { Eye, Trash2, Upload, File as FileIcon, FileText, ImageIcon, InfoIcon } from "lucide-react"
...
export default function FileTable<T extends FieldValues>({
		...
		const handlePaste = (e: ClipboardEvent) => {
			...
			for (let i = 0; i < items.length; i++) {
				if (items[i].type.indexOf("image") !== -1) {
					...
					if (file) {
						// Create a new File with a generated name for pasted images
						const date = new Date()
						...
						const newFile = new File([file], `captura-${timestamp}.png`, { type: file.type })
    ...
    void handleFileChange(dataTransfer.files)
			}
		}
											...
											<div className="flex items-center gap-2">
												<span>
													{file.type?.startsWith("image/") ? (
														...
														<FileIcon className="size-4 text-blue-500 sm:size-5" />
													)}
												</span>
