...
import type { MILESTONE_STATUS } from "@/generated/prisma/enums"
...
> = async ({ queryKey }) => {
	...
	return {
		...raw,
		milestones: raw.milestones.map((m: Milestone) => ({
			...m,
			startDate: m.startDate ? new Date(m.startDate) : null,
			endDate: m.endDate ? new Date(m.endDate) : null,
			activities: m.activities.map((a) => ({
				...a,
				executionDate: new Date(a.executionDate),
			})),
		})),
...
export const useWorkBookMilestones = ({
	workOrderId,
	showAll,
	enabled = true,
}: MilestoneParams) => {
