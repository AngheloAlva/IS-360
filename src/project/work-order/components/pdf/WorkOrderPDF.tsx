							...
							{workOrder.equipment.map((equipment, index) => (
								<View style={styles.tableRow} key={index}>
									...
									<View style={[styles.tableCol, { width: "25%" }]}>
										<Text style={styles.tableCell}>{equipment.location?.path ?? ""}</Text>
									</View>
						...
						<View style={styles.section} key={entry.id}>
							...
							{entry.attachments && entry.attachments.length > 0 && (
								<View style={{ marginTop: 8 }}>
									<Text style={styles.activityLabel}>Evidencia Fotográfica:</Text>
									<View
										style={{
											flexDirection: "row",
											flexWrap: "wrap",
									...
									>
										{entry.attachments
											.filter((att: { sasUrl?: string }) => att.sasUrl)
											.map((att: { sasUrl?: string; name: string }, idx: number) => (
												<View
													key={idx}
													style={{
														width: "48%",
