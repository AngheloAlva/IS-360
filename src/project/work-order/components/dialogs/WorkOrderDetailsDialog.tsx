...
import { WorkRequestStatusLabels } from "@/lib/consts/work-request-status"
						...
						{data?.workRequested && (
							<>
								...
								<div className="flex flex-col gap-2">
									<h2 className="flex items-center gap-2 text-lg">
										<FileTextIcon className="size-5" />
										Solicitud de Trabajo Vinculada
									</h2>
									<div className="rounded-lg border p-3">
										<div className="flex items-center justify-between">
											<p className="text-sm font-semibold">#{data.workRequested.requestNumber}</p>
											<Badge variant="outline">{WorkRequestStatusLabels[data.workRequested.status as keyof typeof WorkRequestStatusLabels] ?? data.workRequested.status}</Badge>
										</div>
										<p className="text-muted-foreground mt-1 text-sm">
											{data.workRequested.description}
										</p>
									</div>
								</div>
							</>
								...
								<div className="grid gap-2">
									{data?.initReport && (
										<Button
											...
											className="flex w-fit items-center gap-2 rounded-lg bg-orange-600/10 px-2 py-1 font-semibold text-orange-600 hover:bg-orange-600 hover:text-white"
										>
