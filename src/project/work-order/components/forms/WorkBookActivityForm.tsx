...
interface ActivityFormProps {
	...
	tutorialUsers?: Array<{ id: string; name: string }>
	tutorialDefaultValues?: Partial<DailyActivitySchema>
	onTutorialSubmit?: (values: DailyActivitySchema) => void
...
export default function ActivityForm({
	...
	const form = useForm<DailyActivitySchema>({
		...
		defaultValues: {
			workOrderId,
			comments: tutorialMode ? tutorialDefaultValues?.comments : "",
			activityName: tutorialMode ? tutorialDefaultValues?.activityName : "",
			activityEndTime: new Date().toTimeString().split(" ")[0],
			activityStartTime: new Date().toTimeString().split(" ")[0],
			executionDate: tutorialMode ? tutorialDefaultValues?.executionDate : startDate,
			milestoneId: tutorialMode ? tutorialDefaultValues?.milestoneId : "",
			personnel: tutorialMode
				? tutorialDefaultValues?.personnel
				: [
						{
							userId: "",
						},
	...
	useEffect(() => {
		...
		void fetchUsers()
	}, [tutorialMode, tutorialUsers, workOrderId])
	...
	async function onSubmit(values: DailyActivitySchema) {
		...
		if (tutorialMode) {
			...
			toast.success("Actividad creada correctamente (simulacion)")
			setOpen(false)
	...
	return (
		<Sheet open={open} onOpenChange={setOpen}>
			...
			<SheetContent data-tutorial-id={contentDataTutorialId} className="gap-0 sm:max-w-xl">
