...
import { useMaintenancePlanTasks } from "@/project/maintenance-plan/hooks/use-maintenance-plans-tasks"
...
interface CreateWorkOrderInitialData {
...
interface CreateWorkOrderFormProps {
	...
	initialData?: CreateWorkOrderInitialData
...
export default function CreateWorkOrderForm({
	...
	const normalizedEquipmentNames = (equipmentName ?? []).filter(Boolean)
	...
	const showTaskSelector = !!workRequestId && !maintenancePlanTaskId
	...
	const { data: tasksData } = useMaintenancePlanTasks({
	...
	useEffect(() => {
		...
		setSelectedCompany(
			companiesData.companies.find((company) => company.id === initialData.companyId)
		)
	}, [companiesData?.companies, initialData?.companyId])
	...
	const form = useForm<WorkOrderSchema>({
		...
		defaultValues: {
			...
			estimatedDays: String(initialData?.estimatedDays ?? "1"),
			estimatedHours: String(initialData?.estimatedHours ?? "8"),
	...
	useEffect(() => {
		const subscription = form.watch((values, { name, type }) => {
			...
			if (name === "estimatedDays" || name === "programDate") {
				...
				if (!Number.isFinite(days) || !values.programDate) return

				const programMidnight = startOfDay(new Date(values.programDate))
				const newEndDate = addDays(programMidnight, days)
			...
			if (name === "estimatedEndDate") {
				...
				const days = differenceInDays(
					startOfDay(new Date(values.estimatedEndDate)),
					startOfDay(new Date(values.programDate))
				)
	...
	async function onSubmit(values: WorkOrderSchema) {
		...
		const resolvedTaskId = maintenancePlanTaskId ?? (selectedTaskId ? [selectedTaskId] : undefined)
		...
		try {
			if (initReportFile && initReportFile.file) {
				...
				const { ok, message } = await createWorkOrder({
					...
					equipmentId,
					workRequestId,
					maintenancePlanTaskId: resolvedTaskId,
				})
   ...
   void queryClient.invalidateQueries({
	...
	return (
		<Sheet open={open} onOpenChange={setOpen}>
			<SheetTrigger
				className={cn(
					"flex h-10 cursor-pointer items-center justify-center gap-1.5 rounded-md bg-white px-3 text-sm font-semibold text-orange-700 transition-all hover:scale-105",
					{
			...
			>
				<PlusCircleIcon className="h-4 w-4 text-current" />
				<span className={buttonLabel ? "inline" : "hidden lg:inline xl:inline"}>
					{buttonLabel ?? (equipmentName ? "Nueva OT" : "Nueva Orden de Trabajo")}
				</span>
			</SheetTrigger>
					...
					>
						...
						{equipmentId && equipmentName ? (
							<FormItem className="sm:col-span-2">
								...
								<Input readOnly name="equipment" value={normalizedEquipmentNames.join(", ")} />
								<ul className="text-muted-foreground mt-2 list-disc space-y-1 pl-4 text-xs">
									{normalizedEquipmentNames.map((name, index) => (
							...
							<>
								...
								<div className="sm:col-span-2">
									<h2 className="text-xl font-bold">
										Asociar a Tarea de Mantenimiento (Opcional)
									</h2>
									<span className="text-muted-foreground text-sm">
										Vincule esta OT con una tarea de un plan de mantenimiento para actualizar
										tiempos y registros.
									</span>
								</div>
								...
								<SelectWithSearch
									...
									options={
										plansData?.maintenancePlans.map((plan) => ({
											value: plan.slug,
											label: `${plan.name} — ${plan.equipment.name}`,
										})) ?? []
									}
									onChange={(value) => {
										setSelectedPlanSlug(value)
										setSelectedTaskId("")
									}}
								/>
