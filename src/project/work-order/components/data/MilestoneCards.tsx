...
interface MilestoneCardsProps {
	...
	requestCloseTriggerDataTutorialId?: string
	requestCloseDialogDataTutorialId?: string
	requestCloseConfirmDataTutorialId?: string
}
...
export default function MilestoneCards({
	...
	requestCloseTriggerDataTutorialId,
	requestCloseDialogDataTutorialId,
	requestCloseConfirmDataTutorialId,
}: MilestoneCardsProps) {
	if (milestones.length === 0) {
		return (
			<div className="flex flex-col items-center justify-center rounded-md border border-orange-600 bg-orange-600/10 p-8 text-center">
				<h3 className="text-lg font-semibold">No hay hitos definidos</h3>
							...
							{milestone.status === MILESTONE_STATUS.IN_PROGRESS &&
								(hassWorkBookPermission || !isInternalMember) && (
									<RequestCloseMilestoneDialog
										...
										onTutorialConfirm={() => onTutorialRequestClose?.(milestone.id)}
										triggerDataTutorialId={requestCloseTriggerDataTutorialId}
										contentDataTutorialId={requestCloseDialogDataTutorialId}
										confirmDataTutorialId={requestCloseConfirmDataTutorialId}
									/>
								)}

							{hasPermission && milestone.status === MILESTONE_STATUS.REQUESTED_CLOSURE && (
								<CloseMilestoneDialog workOrderId={workOrderId} milestoneId={milestone.id} />
							)}
						</CardContent>
					</Card>
				))}
