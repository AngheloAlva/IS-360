		...
		<Sheet open={open} onOpenChange={setOpen}>
			<SheetTrigger asChild>
				<Button
					...
					className={cn(
						"gap-1.5 bg-white font-semibold text-indigo-600 transition-all hover:scale-105 hover:bg-white hover:text-indigo-600",
						className
					)}
				>
					{initialData ? (
													...
													<CommandGroup>
														{isEquipmentsLoading ? (
															...
															equipments?.equipments?.map((equipment) => (
																...
																>
																	{equipment.name + "* (" + (equipment.location?.path ?? "") + ")"}
																	<Check
																		className={cn(
																			"ml-auto text-white",
