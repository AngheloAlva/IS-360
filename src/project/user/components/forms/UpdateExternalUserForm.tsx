...
export default function UpdateExternalUserForm({
				...
				toast("Colaborador actualizado exitosamente", {
					description:
						emailWasChanged && passwordResetEmailSent
							? `El colaborador ${data?.name} fue actualizado y se envió un correo para restablecer contraseña.`
							: `El colaborador ${data?.name} ha sido actualizado exitosamente`,
					duration: 3000,
				})
				form.reset()
				setIsOpen(false)
    void queryClient.invalidateQueries({
				...
				toast("Error al actualizar el colaborador", {
					description: message || "Ocurrió un error al intentar actualizar el colaborador",
					duration: 5000,
				})
			}
		} catch (error) {
