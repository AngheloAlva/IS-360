...
export default function CreateUsersForm({
	...
	async function onSubmit(values: PartnerUsersSchema) {
		...
		try {
			const result = await createPartnerUsers({
				companyId,
				users: values.employees,
			})

			if (result.errorCount > 0) {
				const errorMessages = result.results
					.filter((r) => !r.ok)
					.map((r) => r.message ?? `Error al crear ${r.name}`)
				toast.error("Errores al crear colaboradores", {
			...
			toast.success("Colaboradores creados exitosamente", {
				description: `${result.successCount} colaboradores han sido creados y se les ha enviado un correo con sus credenciales.`,
				duration: 5000,
			})
			setIsOpen(false)
			form.reset()
			...
			toast.error("Error al crear colaboradores", {
				description: "Ocurrió un error al intentar crear los colaboradores",
				duration: 5000,
			})
		} finally {
	...
	return (
		<Sheet open={isOpen} onOpenChange={setIsOpen}>
			<SheetTrigger
				className={cn(
					"flex h-10 items-center justify-center gap-1.5 rounded-lg bg-white px-3 text-sm font-semibold text-blue-600 transition-all hover:scale-105",
					className
				)}
