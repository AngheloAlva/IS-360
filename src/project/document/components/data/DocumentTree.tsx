...
export function DocumentTree({ area, className }: DocumentTreeProps) {
	...
	const { expandedNodes, loadedNodes, loadingNodes, toggleNode, setFileLoading, clearFileLoading } =
		useFetchTreeData(areaValue)
	...
	return (
		<div
			className={cn(
				"relative hidden h-full max-h-[88dvh] w-full overflow-y-auto border-r lg:block",
				className
			)}
		>
