...
export function NewFileFormSheet({ area, userId, parentFolderId = null }: NewFileFormProps) {
	...
	const onSubmit = async (values: FileFormSchema) => {
		...
		try {
			...
			await queryClient.invalidateQueries({ queryKey: ["documents-search"] })
			await queryClient.invalidateQueries({ queryKey: ["documents-charts"] })
		} catch (error) {
			console.error(error)
	...
	return (
		<Sheet open={open} onOpenChange={setOpen}>
			<SheetTrigger
				className="flex h-9 items-center justify-center gap-1 rounded-md bg-green-500 px-3 text-sm tracking-wide text-white transition-all hover:scale-105 hover:bg-green-600 dark:bg-green-700 dark:hover:bg-green-800"
			...
			<SheetContent className="w-full max-w-[90dvw] gap-0 overflow-y-scroll sm:max-w-[70dvw] 2xl:max-w-[50dvw]">
