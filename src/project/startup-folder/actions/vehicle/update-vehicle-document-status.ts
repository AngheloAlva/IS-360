...
export async function undoVehicleDocumentReview({ documentIds }: UndoDocumentReviewParams) {
	try {
		...
		const documents = await prisma.vehicleDocument.updateMany({
		...
		const updatedDocuments = await prisma.vehicleDocument.findMany({
			where: { id: { in: documentIds } },
			select: {
				id: true,
				folder: {
		...
		const uniqueTargets = new Set(
			updatedDocuments
				.map((document) => `${document.folder.vehicleId}:${document.folder.startupFolderId}`)
				.filter(Boolean)
		)

		for (const target of uniqueTargets) {
			const [vehicleId, startupFolderId] = target.split(":")
