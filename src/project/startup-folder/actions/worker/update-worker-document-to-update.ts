...
export interface UpdateWorkerDocumentToUpdateParams {
	documentIds: string[]
	startupFolderId: string
}

export async function updateWorkerDocumentToUpdate({
	documentIds,
	startupFolderId,
}: UpdateWorkerDocumentToUpdateParams) {
	try {
		...
		for (const folderId of folderIds) {
			const folderDocuments = await prisma.workerDocument.findMany({
				where: {
					folderId,
				},
				select: {
				...
				await prisma.workerFolder.update({
					where: {
						workerId_startupFolderId: {
							workerId: folderDocuments[0].folder.workerId,
							startupFolderId,
						},
					},
					data: {
		...
		console.error("Error updating worker documents to TO_UPDATE:", error)
		return {
			ok: false,
