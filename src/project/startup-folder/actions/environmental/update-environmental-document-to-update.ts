...
export interface UpdateEnvironmentalDocumentToUpdateParams {
	documentIds: string[]
	startupFolderId: string
}

export async function updateEnvironmentalDocumentToUpdate({
	documentIds,
	startupFolderId,
}: UpdateEnvironmentalDocumentToUpdateParams) {
	try {
		...
		for (const folderId of folderIds) {
			const folderDocuments = await prisma.environmentalDocument.findMany({
				where: {
					folderId,
				},
				select: {
		...
		for (const document of documents) {
			await logActivity({
				...
				entityType: "EnvironmentalDocument",
		...
		console.error("Error updating environmental documents to TO_UPDATE:", error)
		return {
			ok: false,
