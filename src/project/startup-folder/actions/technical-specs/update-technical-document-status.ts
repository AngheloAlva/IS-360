...
export async function undoTechnicalDocumentReview({ documentIds }: UndoDocumentReviewParams) {
	try {
		...
		const documents = await prisma.techSpecsDocument.updateMany({
		...
		const updatedDocuments = await prisma.techSpecsDocument.findMany({
			where: { id: { in: documentIds } },
			select: {
				id: true,
				folder: {
