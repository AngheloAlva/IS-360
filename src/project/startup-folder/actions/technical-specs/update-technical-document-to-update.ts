...
export interface UpdateTechnicalDocumentToUpdateParams {
	documentIds: string[]
	startupFolderId: string
}

export async function updateTechnicalDocumentToUpdate({
	documentIds,
	startupFolderId,
}: UpdateTechnicalDocumentToUpdateParams) {
	try {
		...
		await prisma.techSpecsDocument.updateMany({
			where: {
				id: { in: documentIds },
				folder: {
					startupFolderId,
		...
		for (const folderId of folderIds) {
			const folderDocuments = await prisma.techSpecsDocument.findMany({
				where: {
					folderId,
				},
				select: {
		...
		console.error("Error updating technical documents to TO_UPDATE:", error)
		return {
			ok: false,
