...
import { DocumentCategory, ReviewStatus, type EnvironmentDocType } from "@prisma/client"
...
export const createEnvironmentDocument = async ({
	...
	try {
		...
		const document = await prisma.environmentDocument.update({
			...
			data: {
				...
				type: type as EnvironmentDocType,
				uploadedBy: {
					connect: {
						id: userId,
					},
...
export const updateExpirationDateEnvironmentDocument = async ({
...
export const submitEnvironmentDocumentForReview = async ({
	emails,
	userId,
	folderId,
}: {
