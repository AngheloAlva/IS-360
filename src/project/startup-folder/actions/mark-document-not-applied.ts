...
export async function markStartupFolderDocumentAsNotApplied(
	...
	try {
		...
		const existingRes = await db.query<{ id: string }>(
			`SELECT id FROM "${cfg.documentTable}" WHERE "folderId" = $1 AND type = $2 LIMIT 1`,
			[folder.id, documentType],
		)
		...
		await db.query(
			`INSERT INTO "${cfg.documentTable}"
			 (id, type, name, url, "uploadedAt", category, status, "uploadedById", "folderId")
		...
		if (!recomputeResult.ok) {
			await db.query(`DELETE FROM "${cfg.documentTable}" WHERE id = $1`, [id])
			return {
				ok: false,
