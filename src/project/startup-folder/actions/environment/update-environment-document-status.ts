...
export async function undoEnvironmentDocumentReview({ documentIds }: UndoDocumentReviewParams) {
	try {
		...
		const documents = await prisma.environmentDocument.updateMany({
		...
		const updatedDocuments = await prisma.environmentDocument.findMany({
			where: { id: { in: documentIds } },
			select: {
				id: true,
				folder: {
