...
export interface UpdateEnvironmentDocumentToUpdateParams {
	documentIds: string[]
	startupFolderId: string
}

export async function updateEnvironmentDocumentToUpdate({
	documentIds,
	startupFolderId,
}: UpdateEnvironmentDocumentToUpdateParams) {
	try {
		...
		await prisma.environmentDocument.updateMany({
			where: {
				id: { in: documentIds },
				folder: {
					startupFolderId,
		...
		for (const folderId of folderIds) {
			const folderDocuments = await prisma.environmentDocument.findMany({
				where: {
					folderId,
				},
				select: {
		...
		console.error("Error updating environment documents to TO_UPDATE:", error)
		return {
			ok: false,
