...
import { Car, IdCard, PlusCircleIcon, Trash2 } from "lucide-react"
...
import { StartupFolderType } from "@/generated/prisma/enums"
...
export default function CompanyForm(): React.ReactElement {
	...
	async function onSubmit(values: CompanySchema) {
		...
		try {
			...
			if (values.supervisors && values.supervisors.length > 0) {
				const result = await createPartnerUsers({
					companyId: data.id,
					users: values.supervisors,
				})
   ...
   void queryClient.invalidateQueries({
	...
	return (
		<Sheet open={open} onOpenChange={setOpen}>
			<SheetTrigger
				className="flex h-10 items-center justify-center gap-1.5 rounded-md bg-white px-3 text-sm font-semibold text-blue-500 transition-all hover:scale-105 dark:text-blue-700"
					...
					>
						...
						<div className="flex w-full items-center justify-center gap-2">
							<Button
								...
								className="w-1/2 border-2 border-indigo-600 font-semibold tracking-wider text-indigo-600 transition-all hover:scale-105 hover:bg-indigo-600 hover:text-white"
							>
								Cancelar
							</Button>
