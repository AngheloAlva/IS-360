...
import { MODULES, ACTIVITY_TYPE, LABOR_CONTROL_DOCUMENT_TYPE } from "@prisma/client"
...
export async function createLaborControlDocument(input: CreateLaborControlDocumentInput) {
	...
	return await prisma.laborControlDocument.create({
		data: {
			url,
			uploadById: userId,
			name: documentName,
