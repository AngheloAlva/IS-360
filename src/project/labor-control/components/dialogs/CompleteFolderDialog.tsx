...
import { completeLaborControlFolder } from "../../actions/complete-labor-control-folder"
...
export default function CompleteFolderDialog({ folderId, onSuccess }: CompleteFolderDialogProps) {
	...
	const handleCompleteFolder = async () => {
		...
		const { ok, message } = await completeLaborControlFolder({ startupFolderId: folderId })
