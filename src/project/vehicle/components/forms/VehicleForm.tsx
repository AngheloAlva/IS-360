...
export default function VehicleForm({ vehicleId, companyId }: VehicleFormProps) {
	...
	const form = useForm<VehicleSchema | UpdateVehicleSchema>({
		resolver: zodResolver(isUpdate ? updateVehicleSchema : (vehicleSchema as any)),
		defaultValues: {
			plate: "",
			model: "",
			brand: "",
		...
		<Sheet open={isOpen} onOpenChange={setIsOpen}>
			...
			>
				{vehicleId ? (
					...
					<Button
						size={"lg"}
						className="hover:bg-scale-105 h-10 rounded-lg bg-white font-semibold text-teal-600 transition-all hover:scale-105 hover:text-teal-700 dark:text-teal-800 dark:hover:text-teal-900"
					>
						<PlusCircleIcon className="size-4 text-current" />
					...
					<Form {...form}>
						...
						>
							...
							<Button
								...
								type="button"
								variant="outline"
								className="w-full border-2 border-emerald-800 font-semibold tracking-wide text-emerald-700 transition-all hover:scale-105 hover:bg-emerald-800 hover:text-white"
