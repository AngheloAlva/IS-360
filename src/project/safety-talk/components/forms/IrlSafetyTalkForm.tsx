...
export function IrlSafetyTalkForm() {
	...
	const form = useForm<IrlSafetyTalkSchema>({
		resolver: zodResolver(irlSafetyTalkSchema as any),
		defaultValues: {
			companyId: "",
			employees: [],
		},
		...
		if (employees) {
			form.setValue(
				...
				employees.map((employee) => {
					const talk = employee.safetyTalks[0]
					const rawSessionDate = talk?.inPersonSessionDate || talk?.lastAttemptAt
					const rawExpiresAt = talk?.expiresAt

					return {
						name: employee.name,
						userId: employee.id,
						talksId: talk?.id,
						score: talk?.score?.toString(),
						approved: talk?.status === "PASSED",
						expiresAt: rawExpiresAt ? new Date(rawExpiresAt) : addYears(new Date(), 1),
						sessionDate: rawSessionDate ? new Date(rawSessionDate) : undefined,
					}
				})
			)
		}
										...
										{fields.map((employee, index) => (
											<div key={employee.id} className="space-y-4 py-5">
												...
												<div className="flex items-start gap-2">
													...
													<InputWithPrefixFormField<IrlSafetyTalkSchema>
														min={0}
														max={100}
														prefix="%"
														type="number"
													...
													<SwitchFormField<IrlSafetyTalkSchema>
														...
														itemClassName="w-fit content-start"
