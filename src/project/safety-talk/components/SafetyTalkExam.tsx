		...
		<RadioGroup value={value} onValueChange={onChange}>
			{question.options?.map((option) => (
				<div key={option.id} className="flex items-center space-x-2">
					...
					<Label htmlFor={option.id} className="text-sm font-semibold">
						{option.text}
					</Label>
				</div>
			))}
		...
		<div className="space-y-4">
			{question.matchingPairs?.map((pair, index) => (
				<div key={index} className="space-y-2">
					<Label htmlFor={`${question.id}-${index}`} className="text-sm font-semibold">
						{pair.question}
					</Label>
					<select
...
export function SafetyTalkExam({
	...
	const handleSubmitAllAnswers = useCallback(async () => {
		...
		try {
			...
			const result = await submitSafetyTalkAnswers(attempt.id, category, formattedAnswers)
  ...
  void initializeAttempt()
   ...
   void handleSubmitAllAnswers()
	...
	return (
		<>
			<div className="rounded-lg bg-linear-to-r from-emerald-600 to-sky-700 p-6 shadow-lg">
