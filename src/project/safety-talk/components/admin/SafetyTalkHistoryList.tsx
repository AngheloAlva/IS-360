...
import { type SAFETY_TALK_CATEGORY, type SAFETY_TALK_STATUS } from "@/generated/prisma/enums"
					...
					<TableBody>
						{history.map((talk) => (
							<TableRow key={talk.id}>
								<TableCell>
									<div>
										<p className="font-semibold">{talk.user.name}</p>
										<p className="text-muted-foreground text-sm">{talk.user.email}</p>
									</div>
								</TableCell>
								<TableCell>
